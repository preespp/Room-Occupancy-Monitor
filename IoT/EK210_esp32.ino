// esp32 wifi module
// modified from https://github.com/Enjoy-Mechatronics/ESP32-Web-Server/blob/main/ESP32-Web-Server.ino
// Load Wi-Fi library
#include <WiFi.h>
#include <string>
...
void loop(){
  ...
  int this_len = serialData.length();

  int i = 0;
  char this_char = serialData.charAt(i);

  while (this_char != ' '){
    i++;
    this_char=serialData.charAt(i);
  }

  String max_pp = "";

  if(i==1){
    max_pp = serialData.charAt(0);

  }else{

    for(int m=0;m<i;m++){
      max_pp += serialData.charAt(m);
    }
  }
  ...
  String current_pp = "";
  for(int m=j;m<this_len;m++){
    current_pp += serialData.charAt(m);
  }
